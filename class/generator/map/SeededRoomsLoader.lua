...
function _M:init(data)
	...
	local random_seed = game.level_random_seed
...
function _M:roomAlloc(room, id, lev, old_lev, add_check)
	...
	while tries > 0 do
		local ok = true
		local random_seed = game.level_random_seed
		if game.level_random_seed then 

		local x, y = 15+random_seed, 15+random_seed

		else 
		local new_seed = rng.range(1,30)

		local x, y = 15+new_seed, 15+new_seed
		game.level_random_seed = new_seed
