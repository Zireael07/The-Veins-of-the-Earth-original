...
function _M:init(actor)
	...
	self.c_learned = List.new{width=250, nb_items=#self.list_learned, height = game.h*0.7, list=self.list_learned, fct=function(item) self:use(item) end, select=function(item,sel) self:on_select(item,sel) end, scrollbar=true}
	self.c_avail = List.new{width=250, nb_items=#self.list_avail, height = game.h*0.7, list=self.list_avail, fct=function(item) self:use(item) end, select=function(item,sel) self:on_select(item,sel) end, scrollbar=true}
	self.c_barred = List.new{width=250, nb_items=#self.list_barred, height = game.h*0.7, list=self.list_barred, fct=function(item) self:use(item) end, select=function(item,sel) self:on_select(item,sel) end, scrollbar=true}
