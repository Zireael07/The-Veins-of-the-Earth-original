...
function _M:init()
    self:generateList()
    Dialog.init(self, "Create Terrain", 1, 1)
...
--    if item.unique then
        local n = game.zone:finishEntity(game.level, "terrain", item.e)
    --    n:identify(true)
        game.zone:addEntity(game.level, n, "terrain", game.player.x, game.player.y)
--[[    else
        local n = game.zone:finishEntity(game.level, "terrain", item.e)
...
function _M:generateList()
    ...
    for i, e in ipairs(game.zone.grid_list) do
        if e.name then
